...
export function createSnowfield(cloudElement, amountOfSnowflakes) {
  ...
  snowfieldSVG.classList.add("snowfield");
  ...
  // This lets the loop sleep for x seconds so we don't spawn all snowflakes
  // at the same time
  const sleep = (ms) => new Promise((res) => setTimeout(res, ms));

  (async () => {
    // Never spawn more than 5 snowflakes per cloud
    for (let i = 0; i < Math.min(amountOfSnowflakes / 2, 5); i++) {
      if (
        ...
        // It is set so we know how many snowflakes to remove from global variable
...
export function createSnowflake(snowfieldSVG, cloudObject) {
  ...
  const randomLightBlue = `rgb(${random(130, 140)}, ${random(200, 210)}, ${random(
    230,
    255
  )})`;
  ...
  const snowflakeTimeline = gsap.timeline();

  // Sets snowflake's visual, colour, opacity and position
  snowflakeTimeline.set(snowflakePath, {
    attr: { d: d },
    fill: randomLightBlue,
    opacity: 0,

    // Makes sure snowflake doesn't spawn outside of the cloud
    x: random(cloudObject.width / 4, cloudObject.width - cloudObject.width / 4),

    // Makes sure snowflake doesn't spawn above cloud
    y: random(
  ...
  // Makes snowflake visible
  snowflakeTimeline.to(snowflakePath, {
    duration: 3,
    opacity: 1,
  });

  // Moves snowflake down and rotates it randomly so it
  // doesn't move straight down, sort of like a snowflake IRL
  snowflakeTimeline.to(
    snowflakePath,
    {
      y: `+=${moveToY}`,
  ...
  // Respawns snowflake after some time
  snowflakeTimeline.to(
    snowflakePath,
    {
      delay: lifetime,
  ...
  function respawnSnowflake() {
    ...
    snowflakeTimeline.killTweensOf(snowflakePath);
